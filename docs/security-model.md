...
The official Codex app-server stdio channel has its own bounded JSONL policy: 64 MiB per envelope and 128 MiB for buffered or queued bytes. Native thread hydration responses can legitimately exceed the stricter limits used for browser-facing and loopback request surfaces, so those policies are not shared. The facade still parses and schema-checks each complete envelope before routing it.
...
Effective `threadId` resume/fork sources are quarantined until the lifecycle response is validated. Native inline history and non-empty rollout paths follow Codex precedence: the ignored `threadId` is not quarantined or required to match, and the actual returned identity is validated and pinned. During a quarantine window, pipelined thread operations are rejected, server-initiated requests receive an error, and client responses to server requests are dropped. If a lifecycle response returns a mismatched model, provider, or effective thread identity, the facade records a bounded tombstone and terminates the proxy session.
