...
Phase 2 implements the app-server facade, initial model-catalog routing, official Codex child lifecycle, and authenticated local Responses boundary. Phase 3 adds the Native Messaging relay, authenticated Windows host-to-bridge IPC, `BrowserSessionCoordinator`, explicit-tab Manifest V3 shell, and Windows development package. Phase 4 replaces the fixed Web fixture with the coordinator's dynamic visible-UI catalog and connects a strict text-only Responses adapter to a UI-only ChatGPT DOM adapter. The current increment adds inactive protocol v2 request/lifecycle and agent-session boundaries plus extension-local document-bound consent; it does not activate or advertise a tool-capable route.
...
[ADR 0005](adr/0005-tool-capable-web-protocol-v2.md) defines a future, explicitly activated Web Agent profile. It keeps the official Codex child as the only owner of tool execution, approvals, and sandbox enforcement. The inactive v2 boundaries translate a validated Web proposal into a Responses function-call item and accept a continuation only after the child returns the exact matching function-call output.
...
The implemented protocol and bridge packages contain strict whole-response envelope codecs, a closed request/continuation admission boundary, a certified JSON Schema subset evaluator, an in-memory agent-session coordinator, and a typed one-shot integration adapter. The extension contains a memory-only 15-minute consent lease bound to the exact selected document; navigation, document replacement, disconnect, restart, expiry, or explicit deactivation invalidates it. The boundaries reject malformed or ambiguous data, bind the canonical request and tool manifest, allow only one pending call, and commit before emitting a child-visible call. They expose no tool execution, approval, sandbox, listener, or catalog API.

A round challenge provides correlation and replay resistance only; because the model can read and repeat it, the challenge is not authentication and does not make a tool proposal trusted. Runtime activation remains blocked on v2 Native Messaging/page negotiation, an atomic coordinator-to-DOM transport adapter with renewal wiring, the current Codex-child approval/execution fixture, complete direct-Chrome agent ownership tests, and explicit release approval described in [Protocol v2](protocol-v2.md).
